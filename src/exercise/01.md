...
React cannot detect any variable that we declare using `let`, `const` or `var`
as state and thus does not update the component if they change. To use managed
state, i.e., have React re-render the component when the state changes, we need
to use the useState hook(or function) to create state.

The useState hook takes in a initial value and returns an array of length 2. The
array contains a state variable whose initial value is set to the initial value
passed to useState and a function which can update the state variable.
